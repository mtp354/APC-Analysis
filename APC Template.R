...
df <- read.csv("C:\\Users\\mattp\\OneDrive\\Documents\\Columbia Stuff\\APC Analysis\\S8 GC Incidence Age Year.csv")
...
info_mat <- round(mutinformation(df), 7)

# For exporting to excel via clipboard:
write.table(info_mat, file = "clipboard", sep = "\t", row.names = F, col.names = T)
